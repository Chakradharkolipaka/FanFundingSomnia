...
export default function NFTCard({ nft, onDelete, onDonation, onTotalsChange }: NFTCardProps) {
  ...
  useWatchContractEvent({
    ...
    onLogs(logs: unknown[]) {
      const filteredLogs = logs.filter((log: unknown) => {
        const args = (log as { args?: { tokenId?: bigint } }).args;
        return args?.tokenId === BigInt(tokenId);
      });
      if (filteredLogs.length > 0) {
        ...
        if (onDonation) {
          filteredLogs.forEach((log: unknown) => {
            const args = (log as { args?: { donor?: unknown; amount?: unknown } }).args;
            const donor = args?.donor;
            const amount = args?.amount;

            if (typeof donor === "string" && typeof amount === "bigint") {
