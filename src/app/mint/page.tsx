...
import { getSelectedChain, getSelectedNetworkKey } from "@/constants/networks";
...
export default function MintPage() {
  ...
  const handleMint = async () => {
    ...
    if (chainId !== selectedChain.id) {
      toast({
        title: "Wrong Network",
        description: `Please switch to ${selectedChain.name} (Chain ID: ${selectedChain.id}) in your wallet. (Selected: ${selectedNetworkKey})`,
        variant: "destructive",
      });
      return;
    }

    // Check contract address configured for the selected network
    if (!CONTRACT_ADDRESS) {
      toast({
        title: "Contract Not Configured",
        description:
          selectedNetworkKey === "somnia"
            ? "Missing NEXT_PUBLIC_SOMNIA_CONTRACT_ADDRESS (or legacy NEXT_PUBLIC_CONTRACT_ADDRESS) in .env.local"
            : "Missing NEXT_PUBLIC_ETHERLINK_CONTRACT_ADDRESS (or legacy NEXT_PUBLIC_CONTRACT_ADDRESS) in .env.local",
        variant: "destructive",
      });
      return;
    }
        ...
        <Card className="w-full max-w-lg">
          ...
          <CardContent className="space-y-6">
            ...
            {isConnected && chainId !== selectedChain.id && (
              <div className="bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded-lg p-4 text-sm text-yellow-800 dark:text-yellow-200">
                ⚠️ Wrong network! Please switch to {selectedChain.name} (Chain ID: {selectedChain.id}) in your wallet.
              </div>
            )}
